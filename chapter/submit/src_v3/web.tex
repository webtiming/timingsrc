...
seen through the eyes of an end user browsing the Web with her favorite
...
\emph{HTML5}~\cite{html5}, allowing Web applications to access an  ever-
increasing pool of features such as local storage, geolocation, peer-to-peer
communications, notifications, background execution, media capture, and more.
This section focus on Web technologies that produce or
...
The playbackrate property was motivated by the fast forward and rewind
features of \emph{digital video disc (DVD)} players and previously
\emph{videocasette recorders (VCR)}. It was not meant for precise control of
...
adjusted for synchronization\footnote{In the future, it may be possible to re-
create a seekable stream out of a MediaStream, thanks to the
\emph{MediaRecorder} interface defined in the MediaStream Recording
specification. This specification is not yet stable when this book is
written.}.
